...
const prices: Record<string, LocalPrice> = {
  SA: { amount: 1990, currencyLabel: "ريال سعودي" },
  AE: { amount: 1990, currencyLabel: "درهم إماراتي" },
  BH: { amount: 199, currencyLabel: "دينار بحريني" },
  KW: { amount: 165, currencyLabel: "دينار كويتي" },
  QA: { amount: 1990, currencyLabel: "ريال قطري" },
  OM: { amount: 199, currencyLabel: "ريال عُماني" },
};

const fallback: LocalPrice = { amount: 530, currencyLabel: "دولار أمريكي" };
const storageKey = "dewank_offer_country";

function useSmallWebsitePrice() {
  ...
  useEffect(() => {
    ...
    fetch("https://api.country.is/", { cache: "no-store", signal: controller.signal })
      ...
      .catch(() => {
        // Keep USD fallback when country detection is unavailable.
      });
...
export function SmallWebsitePrice() {
  const price = useSmallWebsitePrice();
