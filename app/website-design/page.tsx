...
const websiteGuides = [
  { href: "/guides/website-design-cost-saudi-arabia", title: "كم تكلفة تصميم موقع إلكتروني في السعودية؟", text: "افهم ما الذي يغيّر السعر وكيف تقارن نطاق التنفيذ قبل طلب عرض." },
  { href: "/guides/why-website-does-not-generate-leads", title: "لماذا موقعك لا يجلب عملاء؟", text: "شخّص الرسالة والثقة والجوال والـCTA والقياس قبل إعادة البناء." },
];
  ...
  return (
    <main className="wd-page">
      ...
      <style>{`
        ...
        .wd-truth-grid-revised { display: grid; grid-template-columns: minmax(0, .9fr) minmax(0, 1.1fr); column-gap: clamp(2rem, 7vw, 7rem); align-items: start; direction: ltr; }
        .wd-truth-primary { grid-column: 2; grid-row: 1; direction: rtl; }
        .wd-truth-primary h2 { margin: 0; }
        .wd-truth-secondary { grid-column: 1; grid-row: 1; direction: rtl; align-self: start; display: grid; gap: 1.25rem; padding-top: 0; }
        .wd-truth-secondary > p { max-width: 50ch; margin: 0; }
        @media (max-width: 760px) { .wd-truth-grid-revised { grid-template-columns: 1fr; row-gap: 1.5rem; } .wd-truth-primary, .wd-truth-secondary { grid-column: 1; grid-row: auto; } .wd-truth-secondary { align-self: start; padding-top: 0; } }
      `}</style>

      <section className="wd-hero shell"><div className="wd-hero-copy"><div className="wd-kicker"><span>WEBSITE DESIGN</span><span>UX / DEVELOPMENT</span></div><h1>موقعك ليس واجهة فقط.<br/><em>إنه جزء من عملية البيع.</em></h1><p>نصمم ونطوّر مواقع للشركات في السعودية والخليج تشرح القيمة، تبني الثقة، وتحوّل الزيارة إلى خطوة واضحة — مع سرعة، تجربة مستخدم، SEO، وتحليلات من البداية.</p><div className="wd-actions"><Link className="button primary" href="/contact">ناقش مشروع موقعك <span>←</span></Link><a className="wd-text-link" href="#process">استعرض طريقة العمل</a></div></div><div className="wd-browser" aria-label="تصور تفاعلي لواجهة موقع احترافي"><div className="wd-browser-top"><div><i></i><i></i><i></i></div><span>dewank.com</span></div><div className="wd-browser-nav"><b>D</b><div><span>الخدمات</span><span>الأعمال</span><span>عن ديوانك</span></div><button type="button">ابدأ مشروعك</button></div><div className="wd-browser-grid"><div className="wd-screen-copy"><small>01 / CLARITY</small><strong>القيمة<br/><em>قبل الزخرفة.</em></strong><p>رسالة واضحة، دليل ثقة، ومسار يقود إلى الإجراء.</p><span className="wd-demo-cta">اكتشف الحل ←</span></div><div className="wd-screen-cards"><article><b>وضوح الرسالة</b><span>ما الذي تقدمه؟</span></article><article><b>دليل الثقة</b><span>لماذا يختارك العميل؟</span></article><article><b>خطوة تالية</b><span>ماذا يفعل الآن؟</span></article></div></div><div className="wd-browser-foot"><span>UX STRATEGY</span><span>RESPONSIVE</span><span>CONVERSION</span></div></div></section>

      <section className="wd-truth"><div className="shell"><span className="wd-label wd-truth-label">[ الفكرة الأساسية ]</span><div className="wd-truth-grid wd-truth-grid-revised"><div className="wd-truth-primary"><h2>الموقع الجميل قد يجذب النظر.<br/><em>الموقع المدروس يحرّك القرار.</em></h2></div><div className="wd-truth-secondary"><p>عندما لا يفهم الزائر ما تقدمه، أو لا يثق، أو لا يعرف الخطوة التالية، تصبح الزيارة رقمًا بلا قيمة.</p><p>لذلك نربط التصميم بالمحتوى وتجربة المستخدم وسرعة الموقع والتتبع؛ ليعمل الموقع مع التسويق والمبيعات بدل أن يبقى كتالوجًا جامدًا.</p></div></div></div></section>
      ...
      <section className="wd-cluster shell" aria-labelledby="wd-cluster-title"><div className="wd-cluster-head"><div><span className="wd-label">[ قبل القرار ]</span><h2 id="wd-cluster-title">افهم احتياج الموقع.<em>ثم اختر النطاق المناسب.</em></h2></div><p>ابدأ بالدليل المناسب، ثم انتقل للخدمة أو العرض الجاهز فقط عندما يكون نطاقه مناسبًا لمرحلتك.</p></div><div className="wd-guide-grid">{websiteGuides.map((guide,index)=><Link className="wd-guide-card" href={guide.href} key={guide.href}><div className="wd-guide-top"><span>دليل 0{index+1}</span><span className="wd-guide-arrow" aria-hidden="true">↗</span></div><h3>{guide.title}</h3><p>{guide.text}</p></Link>)}</div><div className="wd-offer-grid"><Link className="wd-offer-card" href="/offers/landing-page-package"><small>لحملة أو خدمة واحدة</small><strong>صفحة هبوط مخصصة</strong><p>مسار مركز بهدف واحد مع النص والتصميم وربط التواصل والقياس.</p><span className="wd-offer-price"><LandingOfferPrice kind="base"/><span className="wd-offer-link">شاهد العرض ↗</span></span></Link><Link className="wd-offer-card" href="/offers/small-business-website"><small>لموقع شركة صغير</small><strong>موقع حتى 5 صفحات</strong><p>بداية جاهزة للشركات التي تحتاج حضورًا احترافيًا من دون مشروع تطوير كبير.</p><span className="wd-offer-price"><SmallWebsitePrice/><span className="wd-offer-link">شاهد العرض ↗</span></span></Link></div></section>
